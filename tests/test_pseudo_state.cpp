...
struct PseudoMachine
    : hfsm::state_machine_def<PseudoMachine, MachineState> {

    using IdleState =
        state_entry<Idle, MachineState::Idle>;

    using DecisionState =
        state_entry<Decision, MachineState::Decision>;

    using RouteState =
        state_entry<Route, MachineState::Route>;

    using FastState =
        state_entry<Fast, MachineState::Fast>;

    using SlowState =
        state_entry<Slow, MachineState::Slow>;

    bool should_start() {
        return context.start_requested;
    }

    bool should_run_fast() {
        return context.fast_mode;
    }

    bool should_use_route() {
        return context.slow_mode;
    }

    bool route_ready() {
        return context.route_ready;
    }

    void enter_decision() {
        context.events.push_back("enter_decision");
    }

    void start_fast() {
        context.events.push_back("start_fast");
    }

    void enter_route() {
        context.events.push_back("enter_route");
    }

    void start_slow() {
        context.events.push_back("start_slow");
    }
    ...
    using transition_table = hfsm::mpl::mp_list<
        transition<
            IdleState,
            DecisionState,
            &PseudoMachine::should_start,
            &PseudoMachine::enter_decision
        >,

        transition<
            DecisionState,
            FastState,
            &PseudoMachine::should_run_fast,
            &PseudoMachine::start_fast
        >,

        transition<
            DecisionState,
            RouteState,
            &PseudoMachine::should_use_route,
            &PseudoMachine::enter_route
        >,

        transition<
            RouteState,
            SlowState,
            &PseudoMachine::route_ready,
            &PseudoMachine::start_slow
        >
    >;
};
