...
TEST(hfsm_mpl, tuple_visit_if) {
  ...
  { // visits first matched element
    ...
    const bool found = tuple_visit_if(
        tuple,
        [](auto const& value) { return std::is_same<typename std::decay<decltype(value)>::type, double>::value; },
        [&](auto& value) {
          visited = true;
  ...
  { // returns false when no element matches
    ...
    const bool found = tuple_visit_if(tuple, [](auto const&) { return false; }, [&](auto&) { visited = true; });
  ...
  { // can modify matched element
    ...
    const bool found = tuple_visit_if(tuple, [](int value) { return value == 2; }, [](int& value) { value = 20; });
