...
struct EntryPseudo : hfsm::pseudo_state<EntryPseudo> {
    void on_entry() {
        ++initial_pseudo_context.entry_pseudo_entry_count;
    }

    void on_exit() {
        ++initial_pseudo_context.entry_pseudo_exit_count;
    }
};

struct InitialIdle : hfsm::state<InitialIdle> {
    void on_entry() {
        ++initial_pseudo_context.idle_entry_count;
    }
};

struct InitialPseudoMachine
    : hfsm::state_machine_def<
          InitialPseudoMachine,
          InitialPseudoState> {

    using EntryState =
        state_entry<
            EntryPseudo,
            InitialPseudoState::Entry>;

    using IdleState =
        state_entry<
            InitialIdle,
            InitialPseudoState::Idle>;

    bool should_go_idle() {
        return initial_pseudo_context.go_idle;
    }
    ...
    using transition_table = hfsm::mpl::mp_list<
        transition<
            EntryState,
            IdleState,
            &InitialPseudoMachine::should_go_idle,
            &InitialPseudoMachine::enter_idle
        >
    >;
};
...
protected:
    void SetUp() override {
        initial_pseudo_context.reset();
    }
};

TEST_F(
    InitialPseudoStateTest,
    ResolvesInitialPseudoStateDuringStart) {
...
TEST_F(
    InitialPseudoStateTest,
    DoesNotRemainInPseudoStateAfterStart) {
...
TEST_F(
    InitialPseudoStateTest,
    ThrowsWhenInitialPseudoStateHasNoAvailableTransition) {
...
struct Pseudo1 : hfsm::pseudo_state<Pseudo1> {
    void on_entry() {
        ++pseudo_cycle_context.p1_entry_count;
    }

    void on_exit() {
        ++pseudo_cycle_context.p1_exit_count;
    }
};

struct Pseudo2 : hfsm::pseudo_state<Pseudo2> {
    void on_entry() {
        ++pseudo_cycle_context.p2_entry_count;
    }

    void on_exit() {
        ++pseudo_cycle_context.p2_exit_count;
    }
};

struct PseudoCycleMachine
    : hfsm::state_machine_def<
          PseudoCycleMachine,
          PseudoCycleState> {
    ...
    using IdleState =
        state_entry<
            CycleIdle,
            PseudoCycleState::Idle>;

    using Pseudo1State =
        state_entry<
            Pseudo1,
            PseudoCycleState::Pseudo1>;

    using Pseudo2State =
        state_entry<
            Pseudo2,
            PseudoCycleState::Pseudo2>;

    bool should_enter_cycle() {
        return pseudo_cycle_context.enter_cycle;
    }

    bool pseudo1_to_pseudo2() {
        return pseudo_cycle_context.p1_guard;
    }

    bool pseudo2_to_pseudo1() {
        return pseudo_cycle_context.p2_guard;
    }
    ...
    using transition_table = hfsm::mpl::mp_list<
        transition<
            IdleState,
            Pseudo1State,
            &PseudoCycleMachine::should_enter_cycle,
            &PseudoCycleMachine::enter_pseudo1
        >,
...
protected:
    void SetUp() override {
        pseudo_cycle_context.reset();
    }
};

TEST_F(
    PseudoStateCycleTest,
    ThrowsWhenPseudoStatesFormTransitionCycle) {
...
TEST_F(
    PseudoStateCycleTest,
    EntersAndExitsPseudoStatesBeforeCycleIsDetected) {
